...
function energy(grid::BitMatrix, J::Float64, h::Float64)
	...
	for (i, j) in Tuple.(CartesianIndices(grid))
		ind_mod(i, N) = mod(i-1, N)+1
		neighbour_sum = J * ((grid[ind_mod(i+1, L), j] + grid[i, ind_mod(j+1, L)] + grid[ind_mod(i-1, L), j] + grid[i, ind_mod(j-1, L)]) * 2 - 4)
		
		# if (i-1)*(j-1)*(i-L)*(j-L)==0
		# 	ind_mod(i, N) = mod(i-1, N)+1
		# 	neighbour_sum = J * ((grid[ind_mod(i+1, L), j] + grid[i, ind_mod(j+1, L)] + grid[ind_mod(i-1, L), j] + grid[i, ind_mod(j-1, L)]) * 2 - 4)
		# else
		# 	neighbour_sum = J * ((grid[i+1, j] + grid[i, j+1] + grid[i-1, j] + grid[i, j-1]) * 2 - 4)
		# end
